<template>
  <div class="space-y-8">
    ...
    <div class="space-y-2">
      ...
      <p class="text-lg text-muted-foreground max-w-3xl">
        ...
        <span class="font-medium text-gray-900">{{ assessment.title || 'Untitled Assessment' }}</span>
      </p>
    </div>
    ...
    <div class="rounded-xl bg-card shadow-sm border   overflow-hidden">
      <div class="p-6 sm:p-8">
        <div class="flex items-start justify-between">
          <div class="space-y-2">
            <h2 class="text-2xl font-semibold ">{{ assessment.title || 'Untitled Assessment' }}</h2>
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <!-- Dictionary Stats -->
      <div v-if="assessment.person_dictionary_snapshot" class="overflow-hidden rounded-lg border-accent bg shadow-sm border ">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dt class="truncate text-sm font-medium ">Person Dictionary for referencing</dt>
              <dd class="flex items-baseline">
                <p class="text-xl font-semibold">{{ assessment.person_dictionary_snapshot.name || 'Untitled' }}</p>
              </dd>
              <p class="mt-1 text-sm text-gray-500">{{ assessment.person_dictionary_snapshot.data.length }} entries</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- OMR Sheet Stats -->
      <div v-if="assessment.omr_sheet_snapshot" class="overflow-hidden rounded-lg border-accent bg-card shadow-sm border ">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              ...
              <dd class="flex items-baseline">
                <p class="text-2xl font-semibold ">{{ Object.keys(assessment.omr_sheet_snapshot).length }}</p>
              </dd>
            </div>
          </div>
        </div>
        ...
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              ...
              <dd v-if="localShareCode" class="space-y-1">
                <div class="flex items-center">
                  <span class="text-lg font-mono">{{ localShareCode.code }}</span>
                  <span :class="['ml-2 px-2 py-0.5 rounded-full text-xs font-medium', localShareCode.enabled ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800']">
                    {{ localShareCode.enabled ? 'Active' : 'Disabled' }}
                  </span>
                </div>
                <p class="text-sm text-gray-500">
                  Expires: {{ localShareCode.expires_at ? new Date(localShareCode.expires_at).toLocaleString() : 'Never' }}
                </p>
              </dd>
  ...
  <div class="space-y-5">
        ...
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-sm text-muted-foreground">Enter or update assessment responses</p>
            </div>
          </div>
          <div class="mt-5">
            <a :href="route('assessment.record.view', [assessment.id])"
              class="w-full flex justify-center items-center px-4 py-2 rounded-md shadow-sm text-sm font-medium bg-blue-600 text-white hover:bg-primary/90 transition">
              Go to Recording
            </a>
          </div>
        </div>
        ...
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-sm text-muted-foreground">Manage access to this assessment</p>
            </div>
          </div>
          ...
          <div class="mt-5 grid grid-cols-2 gap-3">
            ...
            <button @click="localShareCode?.enabled ? copyAssistantLink() : null"
              :disabled="!localShareCode?.enabled"
              :class="[
      ...
      <div class="overflow-hidden rounded-lg bg-card shadow-sm border border-border">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <h3 class="text-base font-medium text-foreground">Export Data</h3>
    ...
    <div v-if="showShareCodeModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div class="bg-card border border-border rounded-xl p-6 w-full max-w-md shadow-xl space-y-4">
        ...
        <div class="flex justify-end gap-2 pt-2">
          <button @click="showShareCodeModal = false"
                  class="px-4 py-2 text-sm bg-muted rounded hover:bg-muted/70 text-foreground">Cancel</button>
          <button @click="submitShareCode"
                  class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded">Generate</button>
        </div>
      </div>
    </div>
...
async function submitShareCode() {
  ...
  try {
    const response = await fetch(route('assessment.code.generate', [props.assessment.id]), {
      method: 'POST',
      headers: {
    ...
    console.error(error)
    alert('There was an error generating the share code.')
  }
}

async function disableShareCode() {
  try {
    const response = await fetch(route('assessment.code.disable', [props.assessment.id]), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
...
async function getLatestShareCode() {
  try {
    ...
    alert('There was an error fetching the share code.')
  }
}

function copyAssistantLink() {
  const url = route('assessment.record.assistant.view', {
  ...
  navigator.clipboard.writeText(url)
    ...
    .catch(() => {
      alert('Failed to copy assistant link.')
    })
}
