...
<body class="bg-gray-100 h-screen font-sans p-5">
...
$(document).ready(function () {
    let $paper = $('.pdf-grid-wrapper');
    let $downloadBtn = $("#downloadPdfBtn")
    const omrData = @json($decoded);
    let omrSheetPaperType = omrData.OMRSheet.Format
    ...
    $downloadBtn.on('click', function() {
        ...
        const pdfOptions = {
            ...
            jsPDF: {
                unit: 'in',
                format: omrSheetPaperType.toLowerCase(),
                orientation: 'portrait'
            }
        };

        const paperHtmlString = $paper.prop('outerHTML');
        window.finalLayout = paperHtmlString;
        ...
        setTimeout(() => {
            html2pdf().set(pdfOptions).from($paper[0]).save().then(() => {
                ...
                $spinner.remove(); // remove spinner if error
            });
        }, 100);
    });
});
